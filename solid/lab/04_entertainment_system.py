...
class Television(RCAConnectorMixin, HDMIConnectorMixin, PowerOutletMixin):
...
class DVDPlayer(HDMIConnectorMixin, PowerOutletMixin):
...
class GameConsole(HDMIConnectorMixin, EthernetConnectorMixin, PowerOutletMixin):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(EthernetConnectorMixin, PowerOutletMixin):
    def connect_to_tv(self, television):
